...
def compare_pqr(pqr1_path, pqr2_path, compare_resnames=False):
    ...
    if compare_resnames:
        ...
        if any(diff_resname):
            ...
            raise ValueError(
                "PQR files have different residue names\n"
                f"{df1.res_name[diff_resname]}\n"
                "reference\n"
                f"{df2.res_name[diff_resname]}"
            )
...
def run_propka_for_tests(input_pdb, compare_file, pH):
    ...
    if diff_new:
        raise RuntimeError(
            f"New pKa groups {' '.join(diff_new)} were generated by test which didn't exist in the "
    ...
    for key in results:
        if abs(results[key] - compare[key]) > 1e-3:
            raise RuntimeError(
                f"PROPKA test error. pKa values changed more than 1e-3 for group {key}: new "
                f"{results[key]} reference {compare[key]}"
            )


def run_pkaani_for_tests(input_pdb, compare_file, pH):
    from propka.lib import build_parser

    from pdb2pqr import io
    from pdb2pqr.main import run_pkaani, setup_molecule

    propka_parser = build_parser()
    propka_parser.add_argument(
        "--keep-chain", action="store_true", default=False
    )
    pka_args = propka_parser.parse_args(
        ["--log-level", "WARNING", "--pH", str(pH), "TEST"]
    )
    ...
    biomolecule.remove_hydrogens()
    pka_df = run_pkaani(pka_args, biomolecule)
    results = {
        f"{row['res_name']} {row['res_num']} {row['chain_id']}": row["pKa"]
        for row in pka_df
    }
    ...
    for key in results:
        if abs(results[key] - compare[key]) > 1e-3:
            raise RuntimeError(
                f"pKa-ANI test error. pKa values changed more than 1e-3 for group {key}: new "
                f"{results[key]} reference {compare[key]}"
            )
