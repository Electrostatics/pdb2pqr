...
# modified the original code, which took a string argument for a file name
# now the input is a stream that the program reads from
# Loads the libraries and calculates pKa by passing the atomic environment vector
# found by torch-ANI into the model for the given titratable residue.
def calculate_pka(fileObj):
...
https://doi.org/10.1039/d1sc05610g.\n\n
    ...
    # FEATURES
    tyr_features = joblib.load(
        os.path.join(os.path.dirname(__file__), "FTYR.joblib")
    )
    asp_features = joblib.load(
        os.path.join(os.path.dirname(__file__), "FASP.joblib")
    )
    glu_features = joblib.load(
        os.path.join(os.path.dirname(__file__), "FGLU.joblib")
    )
    lys_features = joblib.load(
        os.path.join(os.path.dirname(__file__), "FLYS.joblib")
    )
    his_features = joblib.load(
        os.path.join(os.path.dirname(__file__), "FHIS.joblib")
    )

    # MODELS
    asp_model = joblib.load(
        os.path.join(
            os.path.dirname(__file__), "ASP_ani2x_FINAL_MODEL_F100.joblib"
        )
    )
    glu_model = joblib.load(
        os.path.join(
            os.path.dirname(__file__), "GLU_ani2x_FINAL_MODEL_F75.joblib"
        )
    )
    his_model = joblib.load(
        os.path.join(
            os.path.dirname(__file__), "HIS_ani2x_FINAL_MODEL_F100.joblib"
        )
    )
    lys_model = joblib.load(
        os.path.join(
            os.path.dirname(__file__), "LYS_ani2x_FINAL_MODEL_F25.joblib"
        )
    )
    tyr_model = joblib.load(
        os.path.join(
            os.path.dirname(__file__), "TYR_ani2x_FINAL_MODEL_F25.joblib"
        )
    )
    ...
    atoms = read_proteindatabank(fileObj, read_arrays=True)
    ...
    pka_res_chain = list(zip(pkares, pkach))
    ...
    for i, r in enumerate(atom_list):
        ...
        if r in pka_res_chain:
            ...
            pkadict[lrnum] = lres, estimate_pka[0]
